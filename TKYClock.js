...
class TKYClock extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            date: this.setHour(9),
    ...
    tick(delay) {
        this.intervalID = setInterval(() => { this.setState({ date: this.setHour(9)}) },
        delay)
    ...
    updateMeNow() {
       this.setState({date: this.setHour(9)})
    }

    render() {
        return (
            <div>
                <h1> TKY timezone - Updateinterval = {this.state.delay/1000} sec </h1>
