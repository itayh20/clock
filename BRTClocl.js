...
class BRTClock extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            date: this.setHour(-3),
    ...
    tick(delay) {
        this.intervalID = setInterval(() => { this.setState({ date: this.setHour(-3)}) },
        delay)
    ...
    updateMeNow() {
       this.setState({date: this.setHour(-3)})
    }

    render() {
        return (
            <div>
                <h1> BRT timezone - Updateinterval = {this.state.delay/1000} sec </h1>
